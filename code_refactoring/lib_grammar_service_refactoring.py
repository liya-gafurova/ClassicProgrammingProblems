...
def _check_auxiliary_verbs(words: List[str], sentence: str, sent_decontracted: str,
                           tagged_words: List[Tuple[Any, Any, Any]]) -> Tuple[List[SingleResult], str]:
    possible_errors = []
    ...
    for i, (word, pos, dependency) in enumerate(tagged_words):
        if pos == 'VERB':
            ...
            if not _first_level_verb_check(word, results):
                ...
                corrected_text = correct(str_for_typos_checking, matches)
                ...
                verb_form_predictions = {i: word_info for i, word_info in
                                         enumerate(happy_roberta.predict_mask(compiled_str, options=list(verb_forms),
...
async def additional_check_with_roBERTa(text, source_text) -> AdditionalChecking:
    ...
    for i, sentence in enumerate(real_text):
        ...
        possible_aux_errors, fixed_sentence = _check_auxiliary_verbs(words, real_sentences[i], sent_decontracted,
                                                                     tagged_words)
        ...
        if fixed_sentence is not None:
            ...
            possible_verb_errors, usage_hints = await _check_verbs(words_for_verb_checking, fixed_sentence,
                                                                   sent_decontracted,
        ...
        usage_hints_all_verbs = await _check_word_usage(words, real_sentences[i], sent_decontracted, tagged_words)
        usage_hints_list.extend(usage_hints_all_verbs)

        # add static errors in conditions of
